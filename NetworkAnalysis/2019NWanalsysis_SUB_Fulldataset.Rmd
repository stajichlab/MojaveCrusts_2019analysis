...
```{r warning=FALSE}
set.seed(711)
physeq.prune.rarefy = rarefy_even_depth(physeq.prune, sample.size = 76106, replace = FALSE, trimOTUs = TRUE)
physeq.prune.rarefy
```
...
```{r}
pdf("Biocrust Microbial Community Network Analysis (circlize)Jan31SUB_allFG&BAC.pdf", width = 18, height = 18)
...
###Knufia
```{r}
physeq.prune.rarefy = prune_taxa(taxa_sums(physeq.prune.rarefy) > 10, physeq.prune.rarefy)
physeq.prune.rarefy.Chaetothyriales = subset_taxa(physeq.prune.rarefy, Order == "Chaetothyriales")
physeq.prune.rarefy.Chaetothyriales = subset_taxa(physeq.prune.rarefy.Chaetothyriales, Genus != "NA")
```

```{r}
physeq.prune.rarefy.Chaetothyriales
```

```{r fig.height=5, fig.width=8, fig.align="center"}
taxcom_sitelayer = ggplot(data = psmelt(physeq.prune.rarefy.Chaetothyriales), mapping = aes_string(x = "Site_layer" ,y = "Abundance", fill = "Genus" )) + 
  geom_bar(stat="identity", position="fill") + 
  ggtitle("Chaetothyriales Taxonomic Composition by Site and Layer")+
