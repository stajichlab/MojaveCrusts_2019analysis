...
bad.vs<-V(easi.fun.bac.SF.net)[degree(easi.fun.bac.SF.net) == 0] 
# Remove isolated nodes
net.grph<-delete.vertices(easi.fun.bac.SF.net, bad.vs)
```
...
```{r}
tax.interkingdom <- data.frame(tax_table(FG.BAC.SF.merged))
tax.interkingdom$OTU <- rownames(tax.interkingdom)
```

```{r}
# getting the mean relative abundance of each OTU
interkingdom.Allegan.filtered.ra <- FG.BAC.SF.merged %>%   
...
cbbPalette <- c("#E69F00", "#999999", "#0072B2", "#56B4E9", "#F0E442", "lavenderblush2", "#CC79A7", "#009E73", "midnightblue", "lightgreen", "aquamarine4","#D55E00","brown", "snow3", "black")
...
```{r}
pdf("./New_Network/SF_betweenness_noLabel.pdf", width = 8, height = 5)
between + 
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
              labels = trans_format("log10", math_format(10^.x))) + 
  annotation_logticks(sides="l")
